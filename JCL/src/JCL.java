...
class HelpCommand implements interfaces.Commandable {
    ...
    public void run(String[] args) {
        if (args == null || args.length == 0) {
            List<String> commands = JCL.getCommands();
            Collections.sort(commands);
            for (String command : commands) {
                String description = JCL.getCommandDescription(command);
            ...
            if (JCL.containsCommand(command)) {
                String usage = JCL.getCommandUsage(command);
